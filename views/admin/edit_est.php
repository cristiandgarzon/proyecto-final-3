...
<body class="flex">
    ...
    <section class="flex  flex-col justify-center items-center h-screen  pl-72">
        <form action="/handledb/estudiantes/editar.php" method="POST" class="flex flex-col">
            <?php
            require_once($_SERVER["DOCUMENT_ROOT"] ."/config/database.php");
            $id=$_GET["id"];
            try{
            ...
            <?php $rol=3; $usuario_id=$result["usuario_id"];?>


            <input type="number" hidden value="<?= $rol ?>" name="rol_id">
            <input type="number" hidden value="<?=$usuario_id?>"
            name="usuario_id">

            <Label>DNI</Label>
            <input type="number" name="dni"
                class="border rounded py-2 px-4 focus:outline-none focus:ring focus:border-blue-500" value="<?= $result["usuario_dni"] ?>">

            <Label>Correo electronico</Label>
            <input type="email" name="correo"
                class="border rounded py-2 px-4 focus:outline-none focus:ring focus:border-blue-500" value='<?=$result["correo"]?>'>

            <Label>Nombre(s)</Label>
            <input type="text" name="nombre"
                class="border rounded py-2 px-4 focus:outline-none focus:ring focus:border-blue-500" value='<?=$result["usuario_nombre"]?>'>



            <Label>Dirección</Label>
            <input type="text" name="direccion"
                class="border rounded py-2 px-4 focus:outline-none focus:ring focus:border-blue-500" value='<?=$result["direccion"]?>'>
